export class MathUtils {
    ...
    static applyTPSMapping(rawX, rawY, tps) {
        ...
        let sumX = 0, sumY = 0, n = tps.controlPoints.length;
        for (let i = 0; i < n; i++) {
            const cp = tps.controlPoints[i];
            const u = this.tpsBasis(Math.pow(rawX - cp.x, 2) + Math.pow(rawY - cp.y, 2));
            sumX += tps.weightsX[i] * u; sumY += tps.weightsY[i] * u;
        }
        // Add Affine components: last 3 values in the weight vector
        ...
        return { x: ax1 + ax2 * rawX + ax3 * rawY + sumX, y: ay1 + ay2 * rawX + ay3 * rawY + sumY };
    }
